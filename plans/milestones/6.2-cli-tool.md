...
│   ├── commands/
│   │   ├── validate.ts   # Single email validation
│   │   └── bulk.ts       # Bulk file validation
│   ├── utils/
...
├── README.md
└── bin/
    └── email-validator   # Executable entry
```
...
function printTableResult(result: ValidationResult, verbose: boolean): void {
  ...
  if (verbose) {
    ...
    const { checks } = result;
    checksTable.push(
      ['Syntax', checks.syntax.valid ? chalk.green('✓') : chalk.red('✗'), checks.syntax.message],
      ['Domain', checks.domain.valid ? chalk.green('✓') : chalk.red('✗'), checks.domain.message],
      ['MX Records', checks.mx.valid ? chalk.green('✓') : chalk.red('✗'), checks.mx.message],
...
function printChecks(result: ValidationResult): void {
  const { checks } = result;
  const checkIcon = (valid: boolean) => valid ? chalk.green('✓') : chalk.red('✗');
